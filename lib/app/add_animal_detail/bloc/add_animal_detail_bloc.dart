...
class AddAnimalDetailBloc
  ...
  Stream<AddAnimalDetailState> _updateDiagnoses(UpdateDiagnosis event,) async* {
    final diagnosis = event.diagnosis == event.previousState.diagnosis 
      ? Diagnoses.none
      : event.diagnosis;
    final treatment = !allowTreatmentSelection(event.previousState.healthStatus, diagnosis,)
      ? Treatments.none
      : null;
    
    yield event.previousState.copyWith(diagnosis: diagnosis, treatment: treatment,);
  }

  Stream<AddAnimalDetailState> _updateHealthStatus(UpdateHealthStatus event,) async* {
  ...
  Stream<AddAnimalDetailState> _updateTreatment(UpdateTreatment event,) async* {
  ...
  static bool allowDiagnosisSelection(HealthStates healthStatus) => healthStatus == HealthStates.ill || healthStatus == HealthStates.suspicious;
  static bool allowTreatmentSelection(HealthStates healthStatus, Diagnoses diagnosis) => 
